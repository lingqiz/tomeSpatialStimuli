...
if ~isempty(params.loadMatrix) && ~strcmp(params.loadMatrix,'/Users/Shared/Matlab_toolboxes/vistadisp-dev/Applications2/Retinotopy/standard/storedImagesMatrices/N')
    ...
    m = round(2 * mrVista_angle2pix(params.display, outerRad));
    n = round(2 * mrVista_angle2pix(params.display, outerRad));
...
% Generate 
% ARI'S STUFF
nImages = size(images,3)-1; % for last image
blocks = reshape(sequence,[],4);
blocks = [blocks flipud(blocks)];
blankBlock = ones(14/duration.stimframe,1)*blankInd;
blankStart = ones(4/duration.stimframe,1)*blankInd;
...
straights = [1 3 5 7];
for i = 1:2
    diagSeq = diags(randperm(4));
    straightSeq = straights(randperm(4));
    sequence = [sequence; blankStart];
    sequence = [sequence; blocks(:,diagSeq(1))];
    sequence = [sequence; blocks(:,straightSeq(1))];
    sequence = [sequence; blankBlock];
    sequence = [sequence; blocks(:,diagSeq(2))];
    sequence = [sequence; blocks(:,straightSeq(2))];
    sequence = [sequence; blankBlock];
    sequence = [sequence; blocks(:,diagSeq(3))];
    sequence = [sequence; blocks(:,straightSeq(3))];
    sequence = [sequence; blankBlock];
    sequence = [sequence; blocks(:,diagSeq(4))];
    sequence = [sequence; blocks(:,straightSeq(4))];
    sequence = [sequence; blankBlock];
end
 
neededFrames = 600/duration.stimframe-length(sequence);
sequence = [ones(neededFrames,1)*blankInd; sequence];
