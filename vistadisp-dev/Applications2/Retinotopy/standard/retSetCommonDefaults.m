...
% ************************
    if ischar(params.stimSize),
        params.radius = mrVista_pix2angle(params.display,floor(min(params.display.numPixels)/2));
    else
        params.radius = params.stimSize;			
    end;
    fprintf('[%s]: Stimulus size: %.1f degrees / %d pixels.\n',...
        mfilename,params.radius,mrVista_angle2pix(params.display,2*params.radius));
